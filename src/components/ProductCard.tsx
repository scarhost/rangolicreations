    ...
    >
      ...
      <Link to={`/product/${product.id}`} className="block relative aspect-[4/5] bg-secondary overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          {product.newArrival && (
            <span className="bg-gold text-accent-foreground text-[10px] font-body font-medium px-2 py-0.5 rounded-full">
              NEW
            </span>
          )}
          {discount > 0 && (
            <span className="bg-primary text-primary-foreground text-[10px] font-body font-medium px-2 py-0.5 rounded-full">
              -{discount}%
            </span>
          )}
        </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-1 mb-2">
          {Array.from({ length: 5 }).map((_, i) => (
            <Star key={i} size={11} className={i < Math.floor(product.rating) ? 'text-gold fill-gold' : 'text-border'} />
          ))}
        ...
        {product.sizes && (
          <div className="flex gap-1 mt-2 flex-wrap">
            {product.sizes.slice(0, 3).map(size => (
              <span key={size} className="text-[10px] text-muted-foreground border border-border rounded px-1.5 py-0.5 font-body">
                {size}
              </span>
            ))}
